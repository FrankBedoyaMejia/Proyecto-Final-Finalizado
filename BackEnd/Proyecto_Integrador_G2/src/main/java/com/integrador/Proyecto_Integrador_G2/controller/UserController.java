...
public class UserController {
    ...
    public ResponseEntity<String> UpdateUser(@RequestBody RegisterRequest user) throws ResourceNotFoundException{
        Optional<User> userSearch = userService.validateUser(user.getEmail());
        if (userSearch.isPresent()){
            service.updateCity(user);
            return ResponseEntity.ok("Updated the User with email: " + user.getEmail());
        }else{
            throw new ResourceNotFoundException("Error. does not exist" +
                    "the id: " + user.getEmail() + "associated with a User in the database");
        }
    }
